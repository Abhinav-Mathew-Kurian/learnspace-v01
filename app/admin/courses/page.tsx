...
import { BookOpen, Users, Eye, EyeOff, Tag } from 'lucide-react';
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {loading ? (
        ...
        <>
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5">
          {courses.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map((c) => (
        ...
        <div className="mt-4 bg-white rounded-xl border border-slate-200">
          <Pagination currentPage={page} totalPages={Math.ceil(courses.length / PAGE_SIZE)} totalItems={courses.length} pageSize={PAGE_SIZE} onPageChange={setPage} />
        </div>
        </>
      )}
