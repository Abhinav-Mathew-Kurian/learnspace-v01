...
import { PlayCircle, Lock, BookOpen, Clock, FileText, Download } from 'lucide-react';
        ...
        <div className="space-y-4">
          {isEnrolled ? (
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <h3 className="font-semibold text-slate-800 text-sm mb-3">Course</h3>
              <p className="text-sm text-slate-500 mb-4">{videos.length} video{videos.length !== 1 ? 's' : ''} available</p>
              {firstVideo && (
                <Link
                  href={`/student/courses/${id}/video/${firstVideo._id}`}
                ...
                >
                  <PlayCircle size={16} /> Start / Continue
                </Link>
              )}
            </div>
          ) : (
